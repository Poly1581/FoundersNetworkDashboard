...
def get_sentry_endpoint_stats(request):
    '''
        Endpoint to get Sentry endpoint statistics
        See: https://docs.sentry.io/api/organizations/retrieve-event-counts-for-an-organization/
    '''
    project_id = PROJECT_ID
    time_range = request.GET.get("timeRange", "7d")
    sentry_url = f"{BASE_URI}/organizations/{ORGANIZATION_SLUG}/stats_v2/"
    params = {
    ...
    try:
        ...
        print(f"Error fetching endpoint stats from Sentry: {e}")
        return HttpResponseBadRequest(f"Error fetching endpoint stats: {e}")
    except Exception as error:
        print(f"An unexpected error occurred in get_sentry_endpoint_stats: {error}")
        return HttpResponseBadRequest(f"An unexpected error occurred: {error}")
