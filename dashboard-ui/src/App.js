            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                    ...
                    <Box display="flex" gap={1} flexWrap="wrap">
                        {(data.criticalServices || []).map(s => <Chip key={s} label={s} color="error" />)}
                    </Box>
                </Grid>
            </Grid>
        </CollapsibleCard>
    ...
    return (
        <CollapsibleCard title={textContent.systemHealth.heading}>
            <Grid container spacing={2} mt={1}>
                <Grid item>
                    <Typography>{textContent.systemHealth.overallUptime} <strong>{data.overallUptime}</strong></Typography>
                </Grid>
                <Grid item>
                    <Typography>{textContent.systemHealth.activeAlerts} <strong>{data.activeAlerts}</strong></Typography>
                </Grid>
                <Grid item>
                    <Typography>{textContent.systemHealth.celeryQueue} <strong>{data.celeryQueue}</strong></Typography>
                </Grid>
                <Grid item>
                    <Typography>{textContent.systemHealth.lastFullCheck} <strong>{data.lastFullCheck}</strong></Typography>
                </Grid>
                <Grid item>
                    <Button size="small">{textContent.systemHealth.recentErrors}</Button>
                </Grid>
            </Grid>
        </CollapsibleCard>
    );
                ...
                <TableBody>
                    {integrations.map(i => (
                        <TableRow key={i.name}>
                            ...
                            <TableCell>
                                <Chip label={i.status} color={i.status === 'Healthy' ? 'success' : i.status === 'Degraded' ? 'warning' : 'error'} size="small" />
                            </TableCell>
...
function MemberCommunicationSection({ data }) {
    ...
    return (
        <CollapsibleCard title={textContent.memberCommunication.heading}>
            <List dense>
                <ListItem><ListItemText primary={textContent.memberCommunication.criticalFeatures} secondary={data.criticalFeatures} /></ListItem>
                <ListItem><ListItemText primary={textContent.memberCommunication.memberFacingIssues} secondary={`${data.memberFacingIssues} ${textContent.memberCommunication.affectedSuffix}`} /></ListItem>
...
export default function App() {
    ...
    async function fetchDashboardData() {
        ...
        const data = {
            ...
            systemHealth: {
                overallUptime: '50.0%', activeAlerts: 196, celeryQueue: '988 jobs', lastFullCheck: '7 days ago'
            },
            activeIssues: [
                {
